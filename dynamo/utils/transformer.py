...
class ScheduledEventTransformer(app_commands.Transformer[Dynamo]):
    async def transform(  # noqa: PLR0912
        self,
        ...
        try:
            ...
            result = next(
                (e for e in _events if e.name == value or str(e.id) == value), None
            )
        ...
        if match := re.match(URL_REGEX, value, flags=re.IGNORECASE):
            ...
            result = (
                discord.utils.get(guild.scheduled_events, name=value) if guild else None
            )
            if not result:
                for g in interaction.client.guilds:
...
class StringMemberTransformer(app_commands.Transformer[Dynamo]):
    async def transform(
        ...
        value: Any,
        /,
    ) -> discord.User | discord.Member | str:
