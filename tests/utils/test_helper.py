...
def test_existing_file(temp_dir: Path) -> None:
    ...
    file_path.touch()
    assert helper.resolve_path_with_links(file_path) == file_path.resolve()


def test_existing_folder(temp_dir: Path) -> None:
    folder_path = temp_dir / "existing_folder"
    folder_path.mkdir()
    assert helper.resolve_path_with_links(folder_path, folder=True) == folder_path.resolve()


def test_non_existing_file(temp_dir: Path) -> None:
    file_path = temp_dir / "non_existing_file.txt"
    resolved_path = helper.resolve_path_with_links(file_path)
...
def test_non_existing_folder(temp_dir: Path) -> None:
    ...
    resolved_path = helper.resolve_path_with_links(folder_path, folder=True)
    assert resolved_path.exists()
