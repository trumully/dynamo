...
def test_existing_file(temp_dir: Path) -> None:
    ...
    file_path.touch()
    assert dynamo.utils.helper.resolve_path_with_links(file_path) == file_path.resolve()


def test_existing_folder(temp_dir: Path) -> None:
    folder_path = temp_dir / "existing_folder"
    folder_path.mkdir()
    assert dynamo.utils.helper.resolve_path_with_links(folder_path, folder=True) == folder_path.resolve()


def test_non_existing_file(temp_dir: Path) -> None:
    file_path = temp_dir / "non_existing_file.txt"
    resolved_path = dynamo.utils.helper.resolve_path_with_links(file_path)
...
def test_non_existing_folder(temp_dir: Path) -> None:
    ...
    resolved_path = dynamo.utils.helper.resolve_path_with_links(folder_path, folder=True)
    assert resolved_path.exists()
...
def test_nested_non_existing_path(temp_dir: Path) -> None:
    ...
    resolved_path = dynamo.utils.helper.resolve_path_with_links(nested_path)
    assert resolved_path.exists()
    assert resolved_path.is_file()
